...
; EDX    - buffer
	...
	mov ecx,[esi+uF_MOD_Channels-36]
	push ebx
	push ebp
loop_ch:
	push esi
	mov esi,[ecx+FSOUND_CHANNEL_fsptr] ; load the correct SAMPLE pointer for this channel
	test esi,esi ; if(!fsptr) skip this channel!
	jz MixExit_1
	push edx ; mix buffer
	push edx ; cur. mix buffer pointer
	mov ebx,[ecx+FSOUND_CHANNEL_mixpos]
	...
	push edi
CalculateLoopCount:
	cmp BYTE PTR [ecx+FSOUND_CHANNEL_speeddir],0
	mov edx,[esi+uF_SAMP_loopstart]
	mov eax,[ecx+FSOUND_CHANNEL_mixposlo]
	jne samplesleftbackwards
	; work out how many samples left from mixpos to loop end
	add edx,[esi+uF_SAMP_looplen]
	sub edx,ebx
	ja submixpos
	mov edx,[esi+uF_SAMP__length]
	sub edx,ebx
submixpos:
...
samplesleftbackwards:
	...
	mov ebp,[ecx+FSOUND_CHANNEL_speedlo]
	shrd eax,edx,5
	mov ebx,[ecx+FSOUND_CHANNEL_speedhi]
	shr edx,5
	shrd ebp,ebx,5
	jnz speedok ; divide by 0 check
	mov ebp,FREQ_40HZ_p
	mov DWORD PTR [ecx+FSOUND_CHANNEL_speedlo],FREQ_40HZ_f
speedok:
	div ebp
	xor ebp,ebp
	neg edx
...
if RAMP_NONE
	...
	movzx eax,WORD PTR [ecx+FSOUND_CHANNEL_ramp_count]
	; VOLUME RAMP SETUP
	; Reasons to ramp
	; 1 volume change
	...
	mov edx,[ecx+FSOUND_CHANNEL_leftvolume]
	mov ebp,[ecx+FSOUND_CHANNEL_rightvolume]
if RAMP_NONE
	mov [ecx+FSOUND_CHANNEL_ramp_leftvolume],edx
	mov [ecx+FSOUND_CHANNEL_ramp_rightvolume],ebp
else
	shl ebp,16
	or ebp,edx
	test eax,eax
	...
	; set up a new ramp
	cmp ebp,[ecx+FSOUND_CHANNEL_ramp_LR_target]
	je volumerampclamp ; restore old ramp
volumerampstart:
	; SETUP NEW RAMP
	mov [ecx+FSOUND_CHANNEL_ramp_LR_target],ebp
	shl edx,volumeramps_pow
	xor bp,bp
	sub edx,[ecx+FSOUND_CHANNEL_ramp_leftvolume]
	xor eax,eax
	sar edx,volumeramps_pow
	mov [ecx+FSOUND_CHANNEL_ramp_leftspeed],edx
	jz novolumerampL
	mov al,volumerampsteps
novolumerampL:
	shr ebp,16-volumeramps_pow
	sub ebp,[ecx+FSOUND_CHANNEL_ramp_rightvolume]
	sar ebp,volumeramps_pow
	mov [ecx+FSOUND_CHANNEL_ramp_rightspeed],ebp
	jz novolumerampR
	mov al,volumerampsteps
novolumerampR:
	test eax,eax
	mov [ecx+FSOUND_CHANNEL_ramp_count],ax
	jz volumerampend
volumerampclamp:
	cmp edi,eax
	...
	mov edi,eax
volumerampend:
	mov eax,[ecx+FSOUND_CHANNEL_ramp_leftspeed]
	mov [ebx],eax    ; ramp_leftspeed
	mov eax,[ecx+FSOUND_CHANNEL_ramp_rightspeed]
	...
	; edi : counter
	mov ebx,[ecx+FSOUND_CHANNEL_mixpos]
	lea ebx,[ebx*2+esi+uF_SAMP_buff]
	push esi
	cmp BYTE PTR [ecx+FSOUND_CHANNEL_speeddir],0
	mov esi,[esp+8] ; <- cur. mix buffer
	mov edx,[ecx+FSOUND_CHANNEL_speedhi]
	mov ebp,[ecx+FSOUND_CHANNEL_speedlo]
	je MixLoop16
	; neg edx:ebp
	neg ebp
...
MixLoop16:
	...
	mov edx,[ecx+FSOUND_CHANNEL_mixposlo]
	sub eax,edi
...
if RAMP_NONE
	mov eax,edx
	imul edx,[ecx+FSOUND_CHANNEL_ramp_rightvolume]
	add [esi],edx
	imul DWORD PTR [ecx+FSOUND_CHANNEL_ramp_leftvolume]
	add [esi+4],eax
else
	xchg eax,edx
	mov edi,eax
	imul DWORD PTR [ecx+FSOUND_CHANNEL_ramp_rightvolume]
	...
	add [esi+4],eax
	mov edx,[uFMOD_fopen]
	add [ecx+FSOUND_CHANNEL_ramp_rightvolume],edi ; + cur_ramp_rightspeed
	add [ecx+FSOUND_CHANNEL_ramp_leftvolume],edx  ; + cur_ramp_leftspeed
endif
	pop edx
	xor eax,eax
	add [ecx+FSOUND_CHANNEL_mixposlo],ebp
	pop edi
	adc eax,edx
	add esi,8
	dec edi
	...
	lea eax,[esi+uF_SAMP_buff]
	sub ebx,eax
	shr ebx,1
	mov [ecx+FSOUND_CHANNEL_mixpos],ebx
if RAMP_NONE
	xor edx,edx
else
	; DID A VOLUME RAMP JUST HAPPEN?
	movzx edx,WORD PTR [ecx+FSOUND_CHANNEL_ramp_count]
	test edx,edx
	jz DoOutputbuffEnd
	mov eax,[mmf]
	cdq
	sub [ecx+FSOUND_CHANNEL_ramp_count],ax
	...
	mov [ecx+FSOUND_CHANNEL_ramp_leftspeed],edx
	neg eax
	mov [ecx+FSOUND_CHANNEL_ramp_rightspeed],edx
...
DoOutputbuffEnd:
	cmp [mix_endflag],dl
	jne MixExit
	movzx eax,BYTE PTR [esi+uF_SAMP_loopmode]
	; SWITCH ON LOOP MODE TYPE
	dec eax ; check for normal loop (FSOUND_LOOP_NORMAL = 1)
	jnz CheckBidiLoop
	mov eax,[esi+uF_SAMP_loopstart]
	mov ebp,[esi+uF_SAMP_looplen]
	add eax,ebp
	cmp ebx,eax
	jbe rewind_ok
	sub ebx,eax
...
CheckBidiLoop:
	...
	neg eax
	adc edx,-1
	and [ecx+FSOUND_CHANNEL_mixposlo],edx
	and [ecx+FSOUND_CHANNEL_mixpos],edx
	and [ecx+FSOUND_CHANNEL_fsptr],edx
	jz MixExit
	cmp [ecx+FSOUND_CHANNEL_speeddir],al ; FSOUND_MIXDIR_FORWARDS
	je BidiForward
BidiBackwards:
	mov eax,[esi+uF_SAMP_loopstart]
	neg ebp
	dec eax
	sub ebp,1
	dec BYTE PTR [ecx+FSOUND_CHANNEL_speeddir] ; set FSOUND_MIXDIR_FORWARDS
	sbb eax,ebx
	mov ebx,[esi+uF_SAMP_loopstart]
	add ebx,eax
	cmp eax,[esi+uF_SAMP_looplen]
	jl BidiFinish
BidiForward:
	mov eax,[esi+uF_SAMP_loopstart]
	add eax,[esi+uF_SAMP_looplen]
	lea edx,[eax-1]
	sbb eax,ebx
	neg ebp
	xchg eax,ebx
	sub ebp,1
	adc ebx,edx
	inc BYTE PTR [ecx+FSOUND_CHANNEL_speeddir] ; go backwards
	cmp ebx,[esi+uF_SAMP_loopstart]
	jl BidiBackwards
BidiFinish:
	mov [ecx+FSOUND_CHANNEL_mixposlo],ebp
ChkLoop_OK:
	test edi,edi
	mov [ecx+FSOUND_CHANNEL_mixpos],ebx
...
MixExit_1:
	...
	pop esi
	cmp ecx,[esi+uF_MOD_uFMOD_Ch-36]
	jl loop_ch
	pop ebp
	pop ebx
	ret
...
; ESI != 0
	...
	test eax,eax
	push edx
	movsx eax,BYTE PTR [ecx+uF_SAMP_finetune]
	mov edi,edx
	jz _do_inc
	cdq
	shl edx,1
...
if VIBRATO_OR_VOLSLIDE
	...
	VibratoOrTremolo:
	; cptr+2 = ESI
		movzx ecx,BYTE PTR [esi+uF_CHAN_vibpos-2]
		mov al,[esi+uF_CHAN_wavecontrol-2]
		mov edx,ecx
		add dl,[esi+uF_CHAN_vibspeed-2]
		and edx,3Fh
		and eax,3 ; switch(cptr->wavecontrol&3)
		mov [esi+uF_CHAN_vibpos-2],dl
		jz vibrato_c0
		; C2 : Sqare wave
		rol ecx,27
		sbb edx,edx
	...
	vibrato_c0:
		...
		sub edx,eax
	vibrato_default:
		movsx eax,BYTE PTR [esi+uF_CHAN_vibdepth-2]
...
; cptr+2 = ESI
	...
	mov [esi+uF_CHAN_freqdelta-2],eax
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_FREQ
	ret
...
; cptr+2 = ESI
	if VIBRATO_OR_VOLSLIDE
		push esi
		add esi,uF_CHAN_tremolopos-uF_CHAN_vibpos
		call VibratoOrTremolo
		pop esi
	else
		movzx ecx,BYTE PTR [esi+uF_CHAN_tremolopos-2]
		mov al,[esi+uF_CHAN_wavecontrol-2]
		mov edx,ecx
		add dl,[esi+uF_CHAN_tremolospeed-2]
		and edx,3Fh
		and eax,3 ; switch(cptr->wavecontrol&3)
		mov [esi+uF_CHAN_tremolopos-2],dl
		jz tremolo_c0
		; C2 : Sqare wave
		rol ecx,27
		sbb edx,edx
	...
	tremolo_c0:
		...
		sub edx,eax
	tremolo_default:
		movsx eax,BYTE PTR [esi+uF_CHAN_tremolodepth-2]
		imul edx
		sar eax,5
	endif
	mov [esi+uF_CHAN_voldelta-2],eax
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_VOLUME
	ret
endif ; TREMOLO_ON
...
; cptr+2 = ESI
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_FREQ
	mov eax,[esi+uF_CHAN_freq-2]
	mov ecx,[esi+uF_CHAN_portatarget-2]
	movzx edx,BYTE PTR [esi+uF_CHAN_portaspeed-2]
	shl edx,2
	sub eax,ecx
	jg porta_do_sub
	add eax,edx
...
env_pos      EQU -36
	...
	lea edi,[ecx+uF_INST_PANPoints]
	xor ebx,ebx
if PANENVELOPE_ON
	mov edx,[edi+uF_INST_PANsustain-uF_INST_PANPoints]
	mov [ebp+sustain_loop],edx ; load PANsustain, PANLoopStart and PANLoopEnd
	mov cl,[edi+uF_INST_PANtype-uF_INST_PANPoints]
	movzx edx,BYTE PTR [edi+uF_INST_PANnumpoints-uF_INST_PANPoints]
endif
	or [esi+uF_CHAN_notectrl-2],al ; cptr->notectrl |= control
if PANENVELOPE_ON
	if VOLUMEENVELOPE_ON
		cmp al,FMUSIC_VOLUME ; is it FMUSIC_VOLUME or FMUSIC_PAN?
	endif
	lea eax,[esi+uF_CHAN_envpanstopped-2]
endif
if VOLUMEENVELOPE_ON
	if PANENVELOPE_ON
		jnz pan_or_vol_ok
	...
	add ebx,uF_CHAN_envvol-uF_CHAN_envpan
	mov eax,[edi+uF_INST_VOLsustain-uF_INST_PANPoints]
	mov [ebp+sustain_loop],eax ; load VOLsustain, VOLLoopStart and VOLLoopEnd
	mov cl,[edi+uF_INST_VOLtype-uF_INST_PANPoints]
	movzx edx,BYTE PTR [edi+uF_INST_VOLnumpoints-uF_INST_PANPoints]
	lea eax,[esi+uF_CHAN_envvolstopped-2]
	add edi,uF_INST_VOLPoints-uF_INST_PANPoints
pan_or_vol_ok:
endif
	cmp [eax],dh
	...
	push ecx      ; -> env_type
	push eax      ; -> envstopped
	lea ecx,[esi+ebx+uF_CHAN_envpanpos-2]
	lea eax,[esi+ebx+uF_CHAN_envpandelta-2]
	push eax      ; -> envdelta
	lea eax,[esi+ebx+uF_CHAN_envpan-2]
	push eax      ; -> env_value
	lea eax,[esi+ebx+uF_CHAN_envpanfrac-2]
	lea ebx,[esi+ebx+uF_CHAN_envpantick-2]
	push eax      ; -> valfrac
	mov eax,[ecx]
...
loop_ok:
	...
	mov [ebp+env_next],edi ; get tick at next point
...
switch_volume:
	...
	shl dl,4
	jz vol_z
	mov [esi+uF_CHAN_portaspeed-2],dl
vol_z:
	and BYTE PTR [esi+uF_CHAN_notectrl-2],NOT_FMUSIC_TRIGGER
if PORTATO_OR_VOLSLIDE
	mov eax,[esi+uF_CHAN_period-2]
	mov [esi+uF_CHAN_portatarget-2],eax ; cptr->portatarget = cptr->period
endif
vol_default:
	ret
case_6: ; / case 8
	neg edx
case_7: ; / case 9
	add [esi+uF_CHAN_volume-2],edx
	ret
case_AB:
	mov [esi+eax+uF_CHAN_vibspeed-1],dl
	ret
case_C:
	shl edx,4
	mov [esi+uF_CHAN_pan-2],edx
	xchg eax,edx
case_D:
	neg edx
case_E:
	add [esi+uF_CHAN_pan-2],edx
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_PAN
	ret
endif ; VOLUMEBYTE_ON
...
; cptr+2 = ESI
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_VOLUME
	mov dx,[esi+uF_CHAN_tremorpos-2]
	cmp dl,dh
	jbe inc_pos
	mov ecx,[esi+uF_CHAN_volume-2]
	neg ecx
	mov [esi+uF_CHAN_voldelta-2],ecx
inc_pos:
	add dh,[esi+uF_CHAN_tremoroff-2]
	cmp dl,dh
	jbe Tremor_Ret
	mov dl,-1
Tremor_Ret:
	inc edx
	mov [esi+uF_CHAN_tremorpos-2],dl
	ret
endif ; TREMOR_ON
...
; bpm = ECX
	...
	div ecx
SetBPM_Ret:
	mov DWORD PTR [_mod+uF_MOD_mixer_samplespertick],eax
	ret
...
if XM_FILE_ON
	...
	; buf  : EAX
	; size : EDX
	call DWORD PTR [ebp+4] ; uFMOD_fread
	mov ebp,esp
	xor ecx,ecx
	mov eax,[esi+uF_MOD_mixer_samplespertick]
...
loadxm_ttl_ok:
	...
	movzx ecx,WORD PTR [esi+uF_MOD_numorders]
	movzx ebx,WORD PTR [esi+uF_MOD_numpatternsmem]
	neg ebx
	mov edi,esi
	sbb edx,edx
	and ecx,edx
...
loadxm_for_continue:
	...
	xchg eax,ebx
loadxm_pats_ok2:
	movzx ecx,BYTE PTR [esi+uF_MOD_numinsts]
	imul ecx,uF_INST_size
	mov [esi+uF_MOD_numpatternsmem],ax
	lea edi,[ecx+eax*uF_PAT_size]
	mov eax,edi
	sub edi,ecx
	movzx ecx,BYTE PTR [esi+uF_MOD_numchannels_xm]
if CHK4VALIDITY
	cmp ecx,64
...
loadxm_numchan_ok:
	...
	mov [esi+uF_MOD_numchannels],ecx
	mov ebx,ecx
	shl ebx,7 ; *FSOUND_CHANNEL_size*2 == *uF_CHAN_size
if CHK4VALIDITY
	jz loadxm_R
endif
	...
	call alloc
	lea edx,[eax+ebx]
	mov [esi+uF_MOD_Channels],eax
	mov [esi+uF_MOD_uFMOD_Ch],edx
	mov ebx,uF_CHAN_size ; = FSOUND_CHANNEL_size*2
loop_2:
	mov BYTE PTR [eax+FSOUND_CHANNEL_speedhi],1
	mov [edx+uF_CHAN_cptr],eax
	add eax,ebx
	add edx,ebx
	dec DWORD PTR [ebp+loadxm_count2]
	jnz loop_2
	mov [esi],edx ; uF_MOD_pattern
	add edi,edx
	movzx ecx,WORD PTR [esi+uF_MOD_defaultbpm]
	mov [esi+uF_MOD_instrument],edi
	mov edi,edx
	; bpm : ECX
	call SetBPM
	movzx ecx,WORD PTR [esi+uF_MOD_defaultspeed]
if GLOBALVOL_ON
	push 64
	pop DWORD PTR [esi+uF_MOD_globalvolume]
endif
	mov [esi+uF_MOD_speed],ecx
...
loadxm_load_pats:
	...
	mov ecx,[esi+uF_MOD_numchannels]
	cmp eax,10000h
	mov [edi],eax
	movzx eax,ax
...
loadxm_ldpats_continue:
	...
	mov ebx,[esi] ; uF_MOD_pattern
	mov eax,[esi+uF_MOD_numchannels]
	push esi
	lea esi,[ebx+ecx*uF_PAT_size]
	lea edi,[eax+eax*4]
...
loadxm_extrapats_ok:
	xor eax,eax
	mov [esi+uF_MOD_mixer_samplesleft],eax
	mov [esi+uF_MOD_tick],eax
if PATTERNDELAY_ON
	lea edi,[esi+uF_MOD_patterndelay]
	stosd
else
	lea edi,[esi+uF_MOD_nextorder]
endif
	stosd
	stosd
	; Load instrument information
	mov al,[esi+uF_MOD_numinsts]
	test al,al
	jz loadxm_ret1
	mov [ebp+loadxm_count1],al
	mov ebx,[esi+uF_MOD_instrument]
loadxm_for_instrs:
	push 33
...
loadxm_inst_ok:
	...
	; !org : Z
	call uFMOD_lseek
	lea edx,[ebx+uF_INST_VOLfade]
	xor eax,eax
	mov cx,[edx]
	shl ecx,1
	cmp BYTE PTR [edx+uF_INST_VOLnumpoints-uF_INST_VOLfade],2
	mov [edx],cx ; iptr->VOLfade *= 2
	jnb ladxm_voltype_ok
	mov [edx+uF_INST_VOLtype-uF_INST_VOLfade],al
ladxm_voltype_ok:
	cmp BYTE PTR [edx+uF_INST_PANnumpoints-uF_INST_VOLfade],2
	jnb loadxm_PANtype_ok
	mov [edx+uF_INST_PANtype-uF_INST_VOLfade],al
loadxm_PANtype_ok:
	cmp [ebp+loadxm_tmp27],al
	je loadx_for_loadsamp_end
	mov [ebp+loadxm_numpat],eax
	mov [ebp+loadxm_fnumpat],ebx ; uF_INST_sample
loadxm_for_samp:
...
loadx_for_loadsamp:
	...
	xor eax,eax
	mov edx,[esi]
	mov ch,[esi+uF_SAMP_Resved]
	mov cl,[esi+uF_SAMP_bytes]
if CHK4VALIDITY
	test edx,0FFC00000h
	jnz loadxm_R
endif
	add esi,uF_SAMP_buff
if ADPCM_ON
...
loadxm_chk_loop_bidi:
	mov eax,[esi+uF_SAMP_looplen-uF_SAMP_buff]
	mov ecx,[esi+uF_SAMP_loopstart-uF_SAMP_buff]
	add eax,ecx
	cmp BYTE PTR [esi+uF_SAMP_loopmode-uF_SAMP_buff],2 ; LOOP_BIDI
	lea eax,[esi+eax*2]
	jnz loadxm_chk_loop_normal
	mov cx,[eax-2]
	jmp loadxm_fix_loop
loadxm_chk_loop_normal:
	cmp BYTE PTR [esi+uF_SAMP_loopmode-uF_SAMP_buff],1 ; LOOP_NORMAL
	jnz loadxm_loops_ok
	mov cx,[esi+ecx*2]
loadxm_fix_loop:
	mov [eax],cx
...
donote_iptr     EQU -28
	...
	mov eax,[esi+uF_MOD_order-36]
	push ebp
	movzx ebx,BYTE PTR [eax+esi+uF_MOD_orderlist-36]
	mov ebp,esp
	mov eax,[esi+uF_MOD_row-36]
	lea ebx,[ecx+ebx*uF_PAT_size]
	mov ecx,[esi+uF_MOD_numchannels-36]
if PATTERNBREAK_ON
	if PATTERNJUMP_ON
		mov [ebp+donote_jumpflag],ch
	endif
endif
	mul ecx
	lea edi,[eax+eax*4] ; x SIZE FMUSIC_NOTE
	mov edx,[ebx+uF_PAT_data]
	push esi
	test edx,edx
	jz donote_R
	add edi,edx ; mod->pattern[mod->orderlist[mod->order]].data+(mod->row*mod->numchannels)
	sub esp,24
	; Loop through each channel in the row
	shl ecx,7 ; x uF_CHAN_size
	jz donote_R
	push esi
	mov esi,[esi+uF_MOD_uFMOD_Ch-36]
	push ebx
	inc esi
	inc esi
	add ecx,esi
donote_for_channels:
	push ecx
	mov bl,[edi+uF_NOTE_eparam]
	mov al,[edi+uF_NOTE_effect]
	and ebx,0Fh
...
donote_doporta:
	...
	mov cl,[edi+uF_NOTE_number]
	jz donote_rem_note
	dec cl
	js donote_rem_inst
	mov [esi+uF_CHAN_inst-2],cl ; remember the instrument #
donote_rem_inst:
	mov cl,[edi] ; get current note
	dec ecx
	cmp cl,96
	jae donote_rem_note
	mov [esi+uF_CHAN_note-2],cl ; remember the note
donote_rem_note:
	movzx ecx,BYTE PTR [esi+uF_CHAN_inst-2]
	mov eax,[ebp+var_mod]
	imul ecx,uF_INST_size
	add ecx,[eax+uF_MOD_instrument-36]
	movzx eax,BYTE PTR [esi+uF_CHAN_note-2]
	cdq
	mov al,[eax+ecx+uF_INST_keymap]
	cmp al,16
	mov [ebp+donote_iptr],ecx
	jae donote_set_sptr
	mov edx,[ecx+eax*4+uF_INST_sample]
donote_set_sptr:
	test edx,edx
...
if NOTEDELAY_ON
	mov ecx,[esi+uF_CHAN_freq-2]
	mov eax,[esi+uF_CHAN_volume-2]
	mov [ebp+donote_oldfreq],ecx
	mov ecx,[esi+uF_CHAN_pan-2]
...
if TREMOLO_ON
	...
	cmp BYTE PTR [esi+uF_CHAN_recenteffect-2],FMUSIC_XM_TREMOLO
	jne donote_tremolo_vol
	mov ecx,[esi+uF_CHAN_voldelta-2]
	add [esi+uF_CHAN_volume-2],ecx
donote_tremolo_vol:
	mov [esi+uF_CHAN_recenteffect-2],al
endif
	xor ecx,ecx
	mov [esi+uF_CHAN_voldelta-2],ecx
	mov [esi+uF_CHAN_freqdelta-2],ecx
	mov BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_VOLUME_OR_FREQ
	; PROCESS NOTE
	mov cl,[edi] ; note
	dec ecx
	cmp cl,96
	...
	movsx eax,BYTE PTR [edx+uF_SAMP_relative]
	add ecx,eax
	mov eax,[ebp+var_mod]
	mov [esi+uF_CHAN_realnote-2],cl
	; Get period according to realnote and finetune
	test BYTE PTR [eax+uF_MOD_flags-36],1
	je donote_flagsn1
	mov eax,[ebp+donote_sptr]
	movsx eax,BYTE PTR [eax+uF_SAMP_finetune]
	cdq
	shl ecx,6
	sub eax,edx
	sar eax,1
...
donote_flagsn1:
	...
	jne donote_freq_ok
	mov [esi+uF_CHAN_freq-2],eax
donote_freq_ok:
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_TRIGGER
donote_note_ok:
	; PROCESS INSTRUMENT NUMBER
	cmp BYTE PTR [edi+uF_NOTE_number],0
	je donote_zcptr_ok
	mov eax,[ebp+donote_sptr]
	; DESCRIPTION: Reset current channel
	push edi
	push 9
	movzx ecx,BYTE PTR [eax+uF_SAMP_defvol]
	mov [esi+uF_CHAN_volume-2],ecx
	pop ecx
	movzx eax,BYTE PTR [eax+uF_SAMP_defpan]
	mov [esi+uF_CHAN_pan-2],eax
	push 64
	xor eax,eax
	pop DWORD PTR [esi+uF_CHAN_envvol-2]
	push 32
	lea edi,[esi+uF_CHAN_envvoltick-2]
	pop DWORD PTR [esi+uF_CHAN_envpan-2]
	mov DWORD PTR [esi+uF_CHAN_fadeoutvol-2],65536
	; memset(&cptr->envvoltick,0,36)
	rep stosd
	; Retrigger tremolo and vibrato waveforms
	mov cl,[esi+uF_CHAN_wavecontrol-2]
	pop edi
	cmp cl,4Fh
	jge z_tremolopos_ok
	mov [esi+uF_CHAN_tremolopos-2],al ; = 0
z_tremolopos_ok:
	test cl,0Ch
	jnz z_vibpos_ok
	mov [esi+uF_CHAN_vibpos-2],al ; = 0
z_vibpos_ok:
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_VOLUME_OR_PAN
donote_zcptr_ok:
if VOLUMEBYTE_ON
	; PROCESS VOLUME BYTE
	movzx edx,BYTE PTR [edi+uF_NOTE_uvolume]
	; volume : EDX
	; cptr+2 : ESI
	call VolByte
endif
	...
	cmp BYTE PTR [edi+uF_NOTE_effect],FMUSIC_XM_KEYOFF
	jne donote_keyoff_ok
donote_set_keyoff:
	inc BYTE PTR [esi+uF_CHAN_keyoff-2]
donote_keyoff_ok:
	; PROCESS ENVELOPES
if VOLUMEENVELOPE_ON
	mov ecx,[ebp+donote_iptr]
	test BYTE PTR [ecx+uF_INST_VOLtype],1
...
donote_no_voltype:
	...
	cmp BYTE PTR [esi+uF_CHAN_keyoff-2],0
	je donote_volenv_ok
	and DWORD PTR [esi+uF_CHAN_envvol-2],0
donote_volenv_ok:
if PANENVELOPE_ON
	mov ecx,[ebp+donote_iptr]
	test BYTE PTR [ecx+uF_INST_PANtype],1
...
donote_no_pantype:
	...
	movzx eax,WORD PTR [ecx+uF_INST_VOLfade]
	sub [esi+uF_CHAN_fadeoutvol-2],eax
	jns donote_fadevol_ok
	and DWORD PTR [esi+uF_CHAN_fadeoutvol-2],0
donote_fadevol_ok:
	; PROCESS TICK 0 EFFECTS
	movzx eax,BYTE PTR [edi+uF_NOTE_effect]
	dec eax ; skip FMUSIC_XM_ARPEGGIO
	movzx edx,BYTE PTR [edi+uF_NOTE_eparam]
...
if INSTRUMENTVIBRATO_ON
	...
	; cptr+2 : ESI
	call DoFlags
	sub esi,-(uF_CHAN_size)
	pop ecx
	add edi,uF_NOTE_size
	cmp esi,ecx
...
S1_C1:
	...
	mov [esi+uF_CHAN_portaupdown-2],dl
donote_xm_porta_end:
	ret
endif
if PORTATO_ON
...
if PORTATOVOLSLIDE_ON
	...
	mov eax,[esi+uF_CHAN_period-2]
	and BYTE PTR [esi+uF_CHAN_notectrl-2],NOT_FMUSIC_TRIGGER_OR_FRQ
	mov [esi+uF_CHAN_portatarget-2],eax
	ret
endif
endif
if PORTATOVOLSLIDE_ON
...
S1_C4:
	shr edx,4
	jz donote_vib_x_ok
	mov [esi+uF_CHAN_vibspeed-2],dl
donote_vib_x_ok:
	test ebx,ebx
	jz donote_vib_y_ok
	mov [esi+uF_CHAN_vibdepth-2],bl
donote_vib_y_ok:
if VIBRATOVOLSLIDE_ON
	xor eax,eax
else
...
S1_C7:
	shr edx,4
	jz donote_trem_x_ok
	mov [esi+uF_CHAN_tremolospeed-2],dl
donote_trem_x_ok:
	test ebx,ebx
	jz donote_trem_y_ok
	mov [esi+uF_CHAN_tremolodepth-2],bl
donote_trem_y_ok:
	ret
endif
if SETPANPOSITION_ON
...
S1_C9:
	...
	mov [esi+uF_CHAN_sampleoffset-2],edx
donote_soffset_ok:
	mov ecx,[ebp+donote_sptr]
	mov edx,[ecx+uF_SAMP_loopstart]
	add edx,[ecx+uF_SAMP_looplen]
	mov eax,[esi+uF_CHAN_sampleoffset-2]
	cmp eax,edx
	mov ecx,[esi+uF_CHAN_cptr-2]
	jb donote_set_offset
	xor eax,eax
	and BYTE PTR [esi+uF_CHAN_notectrl-2],NOT_FMUSIC_TRIGGER
	mov [ecx+FSOUND_CHANNEL_mixpos],eax
	mov [ecx+FSOUND_CHANNEL_mixposlo],eax
donote_set_offset:
	mov [ecx+FSOUND_CHANNEL_fsampleoffset],eax
	ret
endif
if VOLUMESLIDE_ON
S1_C10:
...
S1_C11:
	mov eax,[ebp+var_mod]
	and DWORD PTR [eax+uF_MOD_nextrow-36],0
	mov [eax+uF_MOD_nextorder-36],edx
if PATTERNBREAK_ON
	inc BYTE PTR [ebp+donote_jumpflag]
donote_set_nextord:
endif
	movzx ecx,WORD PTR [eax+uF_MOD_numorders-36]
	cmp [eax+uF_MOD_nextorder-36],ecx
	jl donote_nextorder_ok
	and DWORD PTR [eax+uF_MOD_nextorder-36],0
donote_nextorder_ok:
	ret
endif
if PATTERNBREAK_ON
S1_C13:
	...
	mov eax,[ebp+var_mod]
	mov [eax+uF_MOD_nextrow-36],ecx
if PATTERNJUMP_ON
	cmp BYTE PTR [ebp+donote_jumpflag],0
	jne donote_nextorder_ok
endif
	mov ecx,[eax+uF_MOD_order-36]
	inc ecx
	mov [eax+uF_MOD_nextorder-36],ecx
if PATTERNJUMP_ON
...
S1_C17:
	...
	mov ecx,[ebp+var_mod]
	mov [ecx+uF_MOD_globalvsl-36],dl
donote_globalvsl_ok:
	ret
endif
if SETENVELOPEPOS_ON
S1_C21:
	...
	lea ebx,[ecx+uF_INST_VOLPoints+4]
	; Search and reinterpolate new envelope position
	movzx ecx,BYTE PTR [ecx+uF_INST_VOLnumpoints]
	xor eax,eax
	cmp dx,[ebx]
...
donote_envwhile:
	...
	ja donote_envwhile
donote_env_endwhile:
	mov [esi+uF_CHAN_envvolpos-2],eax
	; if it is at the last position, abort the envelope and continue last volume
	dec ecx
	cmp eax,ecx
	setnl [esi+uF_CHAN_envvolstopped-2]
	jl donote_env_continue
	movzx eax,WORD PTR [ebx+ecx*4-2]
	mov [esi+uF_CHAN_envvol-2],eax ; cptr->envvol = iptr->VOLPoints[((iptr->VOLnumpoints-1)<<1)+1]
donote_envelope_r:
	ret
donote_env_continue:
	mov [esi+uF_CHAN_envvoltick-2],edx
	...
	xchg eax,ecx
donote_no_tickdiff:
	mov [esi+uF_CHAN_envvoldelta-2],ecx
	mov eax,[esi+uF_CHAN_envvoltick-2]
	sub eax,[ebp+donote_currtick]
	imul ecx
	pop edx
	add eax,edx
	mov [esi+uF_CHAN_envvolfrac-2],eax
	shr eax,16
	mov [esi+uF_CHAN_envvol-2],eax
	inc DWORD PTR [esi+uF_CHAN_envvolpos-2]
	ret
endif
if PANSLIDE_ON
S1_C25:
	jz donote_panslide_ok
	mov [esi+uF_CHAN_panslide-2],dl
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_PAN
donote_panslide_ok:
	ret
endif
...
S1_C33:
	...
	test ebx,ebx
	jz donote_paramy_z1
	mov [esi+uF_CHAN_xtraportaup-2],bl
donote_paramy_z1:
	movzx eax,BYTE PTR [esi+uF_CHAN_xtraportaup-2]
	sub [esi+uF_CHAN_freq-2],eax
donote_paramx_n1:
	dec edx
	jnz donote_paramx_n2
	test ebx,ebx
	jz donote_paramy_z2
	mov [esi+uF_CHAN_xtraportadown-2],bl
donote_paramy_z2:
	movzx eax,BYTE PTR [esi+uF_CHAN_xtraportadown-2]
	add [esi+uF_CHAN_freq-2],eax
donote_paramx_n2:
endif
S2_r:
	ret
...
S2_C1:
	...
	mov [esi+uF_CHAN_fineportaup-2],bl
donote_finepup_ok:
	movzx eax,BYTE PTR [esi+uF_CHAN_fineportaup-2]
	shl eax,2
	sub [esi+uF_CHAN_freq-2],eax
	ret
endif
...
S2_C2:
	...
	mov [esi+uF_CHAN_fineportadown-2],bl
donote_finepdown_ok:
	movzx eax,BYTE PTR [esi+uF_CHAN_fineportadown-2]
	shl eax,2
	add [esi+uF_CHAN_freq-2],eax
	ret
endif
if SETVIBRATOWAVE_ON
S2_C4:
	and BYTE PTR [esi+uF_CHAN_wavecontrol-2],0F0h
	or [esi+uF_CHAN_wavecontrol-2],bl
	ret
endif
if SETFINETUNE_ON
S2_C5:
	mov eax,[ebp+donote_sptr]
	mov [eax+uF_SAMP_finetune],bl
	ret
endif
if PATTERNLOOP_ON
S2_C6:
	...
	mov eax,[ebp+var_mod]
	mov eax,[eax+uF_MOD_row-36]
	mov [esi+uF_CHAN_patlooprow-2],eax
	ret
donote_not_paramy:
	mov cl,[esi+uF_CHAN_patloopno-2]
	dec cl
	jns donote_patloopno_ok
	mov ecx,ebx
donote_patloopno_ok:
	mov [esi+uF_CHAN_patloopno-2],cl
	jz donote_patloopno_end
	mov eax,[esi+uF_CHAN_patlooprow-2]
	mov ecx,[ebp+var_mod]
	mov [ecx+uF_MOD_nextrow-36],eax
donote_patloopno_end:
	ret
endif
if SETTREMOLOWAVE_ON
S2_C7:
	and BYTE PTR [esi+uF_CHAN_wavecontrol-2],0Fh
	shl ebx,4
	or [esi+uF_CHAN_wavecontrol-2],bl
	ret
endif
...
S2_C11:
	...
	mov [esi+uF_CHAN_finevslup-2],bl
donote_finevols_ok:
	movsx eax,BYTE PTR [esi+uF_CHAN_finevslup-2]
	sub [esi+uF_CHAN_volume-2],eax
	ret
endif
if NOTEDELAY_ON
S2_C13:
	...
	mov [esi+uF_CHAN_pan-2],eax
	mov BYTE PTR [esi+uF_CHAN_notectrl-2],0
if SETVOLUME_ON
else
	mov [esi+uF_CHAN_volume-2],edx
	ret
...
S2_C14:
	mov ecx,[ebp+var_mod]
	imul ebx,[ecx+uF_MOD_speed-36]
	mov [ecx+uF_MOD_patterndelay-36],ebx
	ret
endif
...
doeff_current EQU -8
	...
	mov eax,[esi+uF_MOD_order-36]
	push ebp
	mov bl,[eax+esi+uF_MOD_orderlist-36]
	mov eax,[esi+uF_MOD_row-36]
	mul DWORD PTR [esi+uF_MOD_numchannels-36]
	mov edx,[ecx+ebx*uF_PAT_size+uF_PAT_data]
	lea eax,[eax+eax*4] ; x SIZE FMUSIC_NOTE
	test edx,edx
	mov ebp,esp
	jz doeff_R
	...
	mov eax,[esi+uF_MOD_numchannels-36]
	shl eax,7 ; x uF_CHAN_size
	push esi
	mov esi,[esi+uF_MOD_uFMOD_Ch-36]
	inc esi
	inc esi
...
doeff_for_channels:
	push eax
	movzx edi,BYTE PTR [esi+uF_CHAN_inst-2]
	mov edx,[ebp+var_mod]
	imul edi,uF_INST_size
	movzx eax,BYTE PTR [esi+uF_CHAN_note-2]
	add edi,[edx+uF_MOD_instrument-36]
	cdq
	mov al,[eax+edi+uF_INST_keymap]
	cmp al,16
	jae doeff_set_sptr
	mov edx,[edi+eax*4+uF_INST_sample]
doeff_set_sptr:
	test edx,edx
...
if INSTRUMENTVIBRATO_ON
	...
	push edx ; sptr
	mov [esi+uF_CHAN_voldelta-2],ebx
	mov [esi+uF_CHAN_freqdelta-2],ebx
	mov [esi+uF_CHAN_notectrl-2],bl
	; PROCESS ENVELOPES
if VOLUMEENVELOPE_ON
	test BYTE PTR [edi+uF_INST_VOLtype],1
	je doeff_no_voltype
	mov al,FMUSIC_VOLUME
	mov ecx,edi
	; cptr+2   : ESI
...
if PANENVELOPE_ON
	test BYTE PTR [edi+uF_INST_PANtype],1
	je doeff_no_pantype
	mov al,FMUSIC_PAN
	mov ecx,edi
	; cptr+2   : ESI
...
doeff_no_pantype:
	...
	; PROCESS VOLUME FADEOUT
	cmp [esi+uF_CHAN_keyoff-2],bl
	je doeff_fadevol_ok
	movzx eax,WORD PTR [edi+uF_INST_VOLfade]
	sub [esi+uF_CHAN_fadeoutvol-2],eax
	jns doeff_fadevol_ns
	mov [esi+uF_CHAN_fadeoutvol-2],ebx
doeff_fadevol_ns:
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_VOLUME
doeff_fadevol_ok:
if VOLUMEBYTE_ON
	mov eax,[ebp+doeff_current]
	movzx eax,BYTE PTR [eax+uF_NOTE_uvolume]
	mov ecx,eax
	shr eax,4
	and cl,0Fh
	sub al,6
...
doeff_case_volE:
	neg ecx
doeff_case_volD:
	sub [esi+uF_CHAN_pan-2],ecx
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_PAN
	jmp doeff_volbyte_end
if VIBRATO_ON
doeff_case_volB:
	mov [esi+uF_CHAN_vibdepth-2],cl
	; cptr+2 : ESI
	call Vibrato
	jmp doeff_volbyte_end
endif
...
doeff_volbyte_end:
	...
	movzx ebx,BYTE PTR [edx+uF_NOTE_eparam]
	mov ecx,ebx
	and ebx,0Fh ; grab the effect parameter y
	movzx eax,BYTE PTR [edx+uF_NOTE_effect]
...
doeff_s3_brk:
	...
	sub esi,-(uF_CHAN_size)
	pop eax
	add DWORD PTR [ebp+doeff_current],uF_NOTE_size
	cmp esi,eax
...
if VOLUMESLIDE_ON
	...
	movzx ecx,BYTE PTR [esi+uF_CHAN_volslide-2]
	mov eax,ecx
	and eax,0Fh
	shr ecx,4
	jz doeff_volslide_ok
...
if PORTADOWN_ON
S3_C2:
	mov al,[esi+uF_CHAN_portaupdown-2]
DoFreqSlide:
	mov ecx,[esi+uF_CHAN_freq-2]
	lea ecx,[ecx+eax*4]
	cmp ecx,1
...
DoFreqSlide_ok:
	mov [esi+uF_CHAN_freq-2],ecx
	and DWORD PTR [esi+uF_CHAN_freqdelta-2],0
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_FREQ
	ret
endif
if PORTAUP_ON
...
if ARPEGGIO_ON
S3_C0:
	cmp [edx+uF_NOTE_eparam],ch
	jbe doeff_arpeggio_ok
	mov eax,[ebp+var_mod]
	mov eax,[eax+uF_MOD_tick-36]
	push 3
	cdq
	pop edi
	idiv edi
...
doeffs_flagsn1:
if AMIGAPERIODS_ON
	movzx eax,BYTE PTR [esi+uF_CHAN_realnote-2]
	mov edi,eax
	...
	call AmigaPeriod
	sub edi,eax
	mov [esi+uF_CHAN_freqdelta-2],edi
endif
doeffs_enable_freq:
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_FREQ
doeff_arpeggio_ok:
...
if PANSLIDE_ON
S3_C25:
	mov cl,[esi+uF_CHAN_panslide-2]
	mov eax,ecx
	and eax,0Fh
	mov edx,[esi+uF_CHAN_pan-2]
	shr ecx,4
...
S3_C17:
	mov ecx,[ebp+var_mod]
	mov edx,[ecx+uF_MOD_globalvolume-36]
	mov al,[ecx+uF_MOD_globalvsl-36]
	test al,0F0h
	; Slide up takes precedence over down
...
set_global_vol:
	...
	if SETGLOBALVOLUME_ON
		...
		mov [ecx+uF_MOD_globalvolume-36],edx
		ret
	endif
endif
if SETGLOBALVOLUME_ON
...
if NOTEDELAY_ON
	...
	mov ecx,[ebp+var_mod]
	xor eax,eax
	cmp [ecx+uF_MOD_tick-36],ebx
	jne doeff_notectrl_z
	mov edx,[esp+4]
	mov DWORD PTR [esi+uF_CHAN_fadeoutvol-2],65536
	movzx edx,BYTE PTR [edx+uF_SAMP_defvol]
	lea ecx,[eax+9]
	mov [esi+uF_CHAN_volume-2],edx
	mov edx,[esi+uF_CHAN_envvol-2]
	lea edi,[esi+uF_CHAN_envvoltick-2]
	cmp ecx,edx
	sbb edx,edx
	and edx,64
	mov [esi+uF_CHAN_envvol-2],edx
	; memset(&cptr->envvoltick,0,36)
	rep stosd
	; Retrigger tremolo and vibrato waveforms
	mov cl,[esi+uF_CHAN_wavecontrol-2]
...
z2_vibpos_ok:
	mov eax,[esi+uF_CHAN_period-2]
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_VOL_OR_FREQ_OR_TR
	mov [esi+uF_CHAN_freq-2],eax
if VOLUMEBYTE_ON
	mov eax,[ebp+doeff_current]
	movzx edx,BYTE PTR [eax+uF_NOTE_uvolume]
	; volume : EDX
	; cptr   : ESI
	jmp VolByte
else
...
doeff_do_notecut:
	...
	and DWORD PTR [esi+uF_CHAN_volume-2],0
	or BYTE PTR [esi+uF_CHAN_notectrl-2],FMUSIC_VOLUME
doeff_notecut_ok:
	ret
endif
...
if INSTRUMENTVIBRATO_ON
	...
	xor eax,eax
	movzx edx,BYTE PTR [esi+uF_INST_VIBtype]
	lea edi,[ebx+uF_CHAN_ivibpos-2]
...
ivib_case_0:
	...
	sub eax,edx
ivib_end_switch:
	movzx edx,BYTE PTR [esi+uF_INST_iVIBdepth]
	imul edx,eax ; delta *= iptr->iVIBdepth
	movzx eax,BYTE PTR [esi+uF_INST_VIBsweep]
	test eax,eax
	jz sweep_ok
	push edi
	xchg eax,edi
	mov eax,[ebx+uF_CHAN_ivibsweeppos-2]
	imul edx
	idiv edi
...
sweep_ok:
	sar edx,6
	add [ebx+uF_CHAN_freqdelta-2],edx ; cptr->freqdelta += delta>>6
	mov edx,[ebx+uF_CHAN_ivibsweeppos-2]
	inc edx
	cmp edx,eax
	jle sweeppos_ok
	xchg eax,edx
sweeppos_ok:
	movzx eax,BYTE PTR [esi+uF_INST_VIBrate]
	add eax,[edi] ; cptr->ivibpos += iptr->VIBrate
	mov [ebx+uF_CHAN_ivibsweeppos-2],edx
	neg ah
	or BYTE PTR [ebx+uF_CHAN_notectrl-2],FMUSIC_FREQ
	neg ah
	sbb ah,0 ; ivibpos - 256
	mov [edi],eax
endif ; INSTRUMENTVIBRATO_ON
	mov esi,[ebx+uF_CHAN_cptr-2]
	test BYTE PTR [ebx+uF_CHAN_notectrl-2],FMUSIC_TRIGGER
	jz no_trig
	; TRIGGER: allocate a channel
	cmp DWORD PTR [esi+FSOUND_CHANNEL_fsptr],0
	jz no_swap
	; Swap between channels to avoid sounds cutting each other off and causing a click
	mov ecx,esi
	sub ecx,[OFFSET _mod+uF_MOD_Channels]
	...
	mov [ebx+uF_CHAN_cptr-2],edi
	pop ecx
	rep movsd
	; This should cause the old channel to ramp out nicely
	mov [esi+FSOUND_CHANNEL_actualvolume-FSOUND_CHANNEL_fsptr],ecx
	mov [esi+FSOUND_CHANNEL_leftvolume-FSOUND_CHANNEL_fsptr],ecx
	mov [esi+FSOUND_CHANNEL_rightvolume-FSOUND_CHANNEL_fsptr],ecx
	pop esi
no_swap:
	lea edi,[esi+FSOUND_CHANNEL_fsptr]
	mov eax,[esp+flags_sptr]
	stosd ; fsptr
	; START THE SOUND!
	xor eax,eax
	mov edx,[esi+FSOUND_CHANNEL_fsampleoffset]
	stosd ; mixposlo
	stosd ; ramp_leftvolume
	stosd ; ramp_rightvolume
	stosd ; ramp_count, speeddir
	mov [esi+FSOUND_CHANNEL_fsampleoffset],eax
	mov [esi+FSOUND_CHANNEL_mixpos],edx
no_trig:
	xor ecx,ecx
	dec cl ; ecx <- 255
	test BYTE PTR [ebx+uF_CHAN_notectrl-2],FMUSIC_VOLUME
	jz no_volume
	mov eax,[ebx+uF_CHAN_volume-2]
	add eax,[ebx+uF_CHAN_voldelta-2]
	mov edx,eax
	shl eax,8
	sub eax,edx ; eax *= 255
if GLOBALVOL_ON
	imul DWORD PTR [_mod+uF_MOD_globalvolume] ; 6 bit  (64)
else
	shl eax,6
	cdq
endif
	not edx
	and eax,edx
	cmp eax,0FF000h ; 64*255*64
	jle vol_upper_bound_ok
	mov eax,0FF000h
vol_upper_bound_ok:
	mul DWORD PTR [ebx+uF_CHAN_envvol-2]      ; 6 bit  (64)
	mul DWORD PTR [ebx+uF_CHAN_fadeoutvol-2]  ; 16 bit (65536)
	; eax:edx /= (2 * 64 * 64 * 64 * 65536)
	shr edx,3
	mov eax,[esi+FSOUND_CHANNEL_actualpan]
	mov [esi+FSOUND_CHANNEL_actualvolume],edx
	mov edi,edx
	imul edx
	idiv ecx
	mov [esi+FSOUND_CHANNEL_leftvolume],eax ; leftvolume <- volume*actualpan/255
	mov eax,ecx
	sub eax,[esi+FSOUND_CHANNEL_actualpan]
	imul edi
	idiv ecx
	mov [esi+FSOUND_CHANNEL_rightvolume],eax
no_volume:
	test BYTE PTR [ebx+uF_CHAN_notectrl-2],FMUSIC_PAN
	jz no_pan
	mov edi,128
	mov eax,[ebx+uF_CHAN_pan-2]
	sub eax,edi
	cdq
	xor eax,edx
	sub eax,edx
	sub edi,eax
	mov eax,[ebx+uF_CHAN_envpan-2]
	sar edi,5
	sub eax,32
	imul edi
	add eax,[ebx+uF_CHAN_pan-2]
	cdq
	not edx
...
pan_ae255:
	xchg eax,edx
	mov [esi+FSOUND_CHANNEL_actualpan],eax
	mov [esi+FSOUND_CHANNEL_leftvolume],edx
	not al ; 255 - pan
	imul DWORD PTR [esi] ; actualvolume
	idiv ecx
	mov [esi+FSOUND_CHANNEL_rightvolume],eax
no_pan:
	test BYTE PTR [ebx+uF_CHAN_notectrl-2],FMUSIC_FREQ
	jz no_freq
	mov ecx,[ebx+uF_CHAN_freq-2]
	xor edx,edx
	add ecx,[ebx+uF_CHAN_freqdelta-2]
	lea eax,[edx+40] ; f = 40 Hz
	jle freq_bounds_ok
	test BYTE PTR [_mod+uF_MOD_flags],1
	jnz modflags_n1
	mov eax,0DA7790h
	div ecx
	xor edx,edx
...
modflags_n1:
	mov eax,4608
	sub eax,[ebx+uF_CHAN_freq-2]
	add eax,[ebx+uF_CHAN_freqdelta-2]
	push eax
	fild DWORD PTR [esp]
...
freq_bounds_ok:
	...
	div ebx
	mov [esi+FSOUND_CHANNEL_speedhi],eax
	div ebx
	mov [esi+FSOUND_CHANNEL_speedlo],eax
no_freq:
	pop edi
	pop esi
if INSTRUMENTVIBRATO_ON
